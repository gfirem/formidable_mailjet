...
class FormidableMailJetSegmentField {
	...
	public function set_formidable_key_field_options( $fieldData ) {
		if ( $fieldData['type'] == 'mailjet_segment' ) {
			$fieldData['name'] = FormidableMailJetManager::t( "MJ Segment" );
			
			$defaults = array(
				'mailjet_segment_show_deleted' => ''
			);
	...
	public function show_formidable_key_field_admin_field( $field ) {
		...
		<div class="frm_html_field_placeholder">
			<div class="frm_html_field"><?= ( $credential != false ) ? FormidableMailJetManager::t( "Pick a Segment list from MailJet." ) : "<div style='color: #ff0000;'>" . FormidableMailJetManager::t( "Configure your credentials." ) . "&nbsp;" . FormidableMailJetManager::get_setting_link() . "</div>"; ?> </div>
		</div>
	<?php
	}
	...
	public function field_formidable_key_field_option_form( $field, $display, $values ) {
		...
		$show_deletes = "";
		if ( $field['mailjet_segment_show_deleted'] == "1" ) {
			$show_deletes = "checked='checked'";
		}
		?>
		<tr>
			<td>
				<label for="field_options[mailjet_segment_show_deleted_<?php echo $field['id'] ?>]"><?= FormidableMailJetManager::t( "Show delete items" ) ?></label>
				<span class="frm_help frm_icon_font frm_tooltip_icon" title="" data-original-title="<?= FormidableMailJetManager::t( "Check if you want to show deleted segments list" ) ?>"></span>
			</td>
			<td>
				<input type="checkbox" <?= $show_deletes ?> name="field_options[mailjet_segment_show_deleted_<?php echo $field['id'] ?>]" id="field_options[mailjet_segment_show_deleted_<?php echo $field['id'] ?>]" value="1"/>
			</td>
		</tr>
	<?php
	}
	...
	public function show_formidable_key_field_front_field( $field, $field_name ) {
		...
		if ( $credential != false ) {
			...
			$filters   = [
				'IsDeleted' => ( ! empty( $field['mailjet_segment_show_deleted'] ) && $field['mailjet_segment_show_deleted'] == "1" )
			];
			$response  = $mj_client->get( Resources::$Contactfilter, [ 'filters' => $filters ] );
			if ( $response->success() ) {
	...
	public function display_formidable_key_field_admin_field( $value, $field, $atts ) {
		if ( $field->type != 'mailjet_segment' || empty( $value ) ) {
			return $value;
		}
	...
	public function add_formidable_key_field_display_options( $display ) {
		if ( $display['type'] == 'mailjet_segment' ) {
			$display['unique']         = true;
